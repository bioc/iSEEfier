...
# conditions : a charater string of the groups/conditions (as it appears in the colData)


iSEEfier <- function(sce, feature.list, reddim.type="TSNE", panel.width=4L, clusters = colnames(colData(sce))[1], conditions = colnames(colData(sce))[1]) {
  initial <- list()
...
initial <- iSEEfier(sce,test, my_clusters,my_conditions)
